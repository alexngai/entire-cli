 ...
 * Cursor Agent
 *
 * Implementation of the Sessionlog agent interface for Cursor IDE.
...
class CursorAgent implements Agent, HookSupport, TranscriptChunker {
  ...
  async getSessionDir(repoPath: string): Promise<string> {
    if (process.env.SESSIONLOG_TEST_CURSOR_PROJECT_DIR) {
      return process.env.SESSIONLOG_TEST_CURSOR_PROJECT_DIR;
    }
  ...
  async installHooks(repoPath: string, force = false): Promise<number> {
    ...
    for (const { key, hookName } of hookTypes) {
      ...
      if (force) {
        entries = entries.filter((e) => !isSessionlogHook(e.command));
      }

      const cmd = `sessionlog hooks cursor ${hookName}`;
  ...
  async uninstallHooks(repoPath: string): Promise<void> {
    ...
    try {
      ...
      for (const key of Object.keys(hooks)) {
        hooks[key] = (hooks[key] ?? []).filter((e) => !isSessionlogHook(e.command));
  ...
  async areHooksInstalled(repoPath: string): Promise<boolean> {
    ...
    try {
      ...
      for (const key of Object.keys(hooks)) {
        ...
        if (Array.isArray(entries) && entries.some((e) => isSessionlogHook(e.command))) {
          return true;
        }
      }
    } catch {
