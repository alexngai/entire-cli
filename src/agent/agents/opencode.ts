 ...
 * OpenCode Agent
 *
 * Implementation of the Sessionlog agent interface for OpenCode.
...
const SESSIONLOG_PLUGIN_MARKER = 'Auto-generated by `sessionlog enable --agent opencode`';
  ...
  implements Agent, HookSupport, TranscriptAnalyzer, TokenCalculator, TranscriptChunker
{
  ...
  async getSessionDir(repoPath: string): Promise<string> {
    if (process.env.SESSIONLOG_TEST_OPENCODE_PROJECT_DIR) {
      return process.env.SESSIONLOG_TEST_OPENCODE_PROJECT_DIR;
    }
    const projectDir = sanitizePathForOpenCode(repoPath);
    return path.join(os.tmpdir(), 'sessionlog-opencode', projectDir);
  }
  ...
  async installHooks(repoPath: string, force = false): Promise<number> {
    const pluginPath = path.join(repoPath, OPENCODE_DIR, 'plugins', 'sessionlog.ts');

    // Check if already installed
    if (!force) {
      try {
        const content = await fs.promises.readFile(pluginPath, 'utf-8');
        if (content.includes(SESSIONLOG_PLUGIN_MARKER)) return 0;
      } catch {
  ...
  async uninstallHooks(repoPath: string): Promise<void> {
    ...
    try {
      const content = await fs.promises.readFile(pluginPath, 'utf-8');
      if (content.includes(SESSIONLOG_PLUGIN_MARKER)) {
  ...
  async areHooksInstalled(repoPath: string): Promise<boolean> {
    ...
    try {
      const content = await fs.promises.readFile(pluginPath, 'utf-8');
      return content.includes(SESSIONLOG_PLUGIN_MARKER);
    } catch {
      return false;
    }
  }
...
function generateOpenCodePlugin(): string {
  return `// ${SESSIONLOG_PLUGIN_MARKER}
// This plugin integrates OpenCode with Sessionlog for session tracking.
...
const plugin: Plugin = {
  ...
  hooks: {
    "session.start": async ({ session }) => {
      ...
      const proc = Bun.spawn(["sessionlog", "hooks", "opencode", "session-start"], {
    ...
    "session.end": async ({ session }) => {
      const payload = JSON.stringify({ session_id: session.id });
      const proc = Bun.spawn(["sessionlog", "hooks", "opencode", "session-end"], {
        stdin: new Blob([payload]),
        stdout: "ignore",
        stderr: "ignore",
      });
    ...
    "message.create": async ({ message, session }) => {
      if (message.role === "user") {
        ...
        const proc = Bun.spawn(["sessionlog", "hooks", "opencode", "turn-start"], {
          stdin: new Blob([payload]),
          stdout: "ignore",
          stderr: "ignore",
        });
    ...
    "message.complete": async ({ message, session }) => {
      if (message.role === "assistant") {
        const payload = JSON.stringify({ session_id: session.id });
        const proc = Bun.spawn(["sessionlog", "hooks", "opencode", "turn-end"], {
          stdin: new Blob([payload]),
          stdout: "ignore",
          stderr: "ignore",
        });
