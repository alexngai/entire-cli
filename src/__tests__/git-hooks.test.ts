...
describe('Git Hooks (with commit-msg)', () => {
  ...
  it('should generate commit-msg hook with exit 1 on failure', async () => {
    ...
    expect(commitMsgHook).toContain('# Sessionlog CLI hook');
    expect(commitMsgHook).toContain('sessionlog hooks git commit-msg "$1" || exit 1');
  ...
  it('should uninstall all 4 hooks', async () => {
    ...
    // All hooks should be removed (files deleted since they only had Sessionlog content)
